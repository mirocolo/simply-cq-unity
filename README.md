...
- 逻辑层 **222 项自动化自检**，而且**不需要打开 Unity** 就能跑
...
**还没做**：M5 最后一块（NPC 传送员）、音效/特效/平衡打磨（M6）；法师与道士暂缓。
...
| **M3** 成长 | 背包（48 格）、装备 8 部位、属性聚合、掉落表、消耗品（负重已移除） | 🟡 逻辑层完成（222 项用例） |
...
| **M5** 内容 | 3 张图、15~20 种怪、60 件装备、NPC 与传送 | 🟡 多地图与传送 ✅ / 装备品质 ✅ / 60 件装备 ✅ / 20 种怪 ✅ / NPC 传送员待做 |
