...
**还没做**：M5 剩下的内容（15~20 种怪、NPC 传送员）、音效/特效/平衡打磨（M6）；法师与道士暂缓。
...
**这 60 件不要手改**，改 `Tools/gen_items.py` 里的设计表再跑一次（它同时会校验 `monsters.json` 的掉落表）：

```bash
python3 Tools/gen_items.py            # 只校验并打印阶梯，不落盘
python3 Tools/gen_items.py --write    # 写回 items.json + monsters.json 的掉落段
```
...
`drops` 里的装备条目由 `gen_items.py` 生成（按档次摊概率，三种怪分别管一个等级段）；
材料 / 药水那几条是手写的，脚本会原样保留。
...
| **M5** 内容 | 3 张图、15~20 种怪、60 件装备、NPC 与传送 | 🟡 多地图与传送 ✅ / 装备品质 ✅ / 60 件装备 ✅ / 怪与 NPC 待铺 |
...
- **只有 3 种怪**：装备已经铺到 60 件，但会掉装备的只有鸡 / 野狼 / 野猪三种，所以等级段只有 3 段（Lv1/4/7）。
  再加怪就能顺手把装备往上铺（改 `Tools/gen_items.py` 的设计表即可）
