...
- **背包 48 格 + 负重上限**：传奇式「捡了一堆东西走不动」—— 超重就捡不起来，东西留在原地并给一次提示（2 秒节流，不刷屏）
...
| **M3** 成长 | 背包（48 格 + 负重）、装备 8 部位、属性聚合、掉落表、消耗品 | 🟡 逻辑层完成（153 项用例） |
| **M3b** 界面与经济 | 背包面板、角色面板、商店 NPC 买卖、存档 | ⬜ 下一步 |
...
- **背包/装备还没有界面**：逻辑（48 格 + 负重 + 8 部位 + 属性聚合）已经能跑、有 153 项用例守着，但游戏里还看不到也点不了 —— 面板在 M3b
