...
- 逻辑层 **207 项自动化自检**，而且**不需要打开 Unity** 就能跑
...
**还没做**：M5 剩下的内容（15~20 种怪、60 件装备、NPC 传送员）、音效/特效/平衡打磨（M6）；法师与道士暂缓。
...
传送点写在 `Data/maps/*.json` 的 `portals` 里 —— 想加图/改走向只改数据，不用碰代码。
...
### `Data/items.json` —— 物品与装备

```json
{ "id": "wp_long", "name": "长剑", "sprite": "wp_long",
  "type": "equip", "slot": "weapon",
  "price": 900, "levelReq": 7, "minQuality": "green",
  "minDc": 6, "maxDc": 12, "desc": "好东西" }
```
...
| **M3** 成长 | 背包（48 格）、装备 8 部位、属性聚合、掉落表、消耗品（负重已移除） | 🟡 逻辑层完成（207 项用例） |
...
| **M5** 内容 | 3 张图、15~20 种怪、60 件装备、NPC 与传送 | 🟡 多地图与传送 ✅ / 装备品质机制 ✅ / 怪与装备待铺 |
...
- **只剩 3 种怪、13 件装备**：品质机制已经把「60 件装备」的地基打好（填数据就行），但数据本身还没铺
