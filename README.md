...
- 逻辑层 **243 项自动化自检**，而且**不需要打开 Unity** 就能跑
...
**还没做**：音效/特效/UI 皮肤/数值平衡（M6）；法师与道士暂缓。**M5 内容部分已全部完成。**
...
| **M3** 成长 | 背包（48 格）、装备 8 部位、属性聚合、掉落表、消耗品（负重已移除） | 🟡 逻辑层完成（243 项用例） |
