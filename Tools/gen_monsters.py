...
MONSTERS = [
    # ---- 草原 · 低级带（掉 b1 段装备）----
    ("mon_hen",         "鸡",       1,  24,  0,  1,  2,   5, 7, 16, 1,  4, False,  6, 0,  0,   2, 0.60),
    ("mon_rabbit",      "野兔",     1,  18,  0,  1,  2,   5, 9, 18, 1,  4, False,  8, 0,  0,   2, 0.60),
    ("mon_deer",        "鹿",       2,  34,  1,  2,  3,   9, 6, 15, 1,  5, False,  8, 0,  1,   4, 0.80),
    ("mon_scorpion",    "毒蝎",     3,  40,  2,  3,  5,  14, 5, 14, 1,  6, True,  10, 0,  2,   6, 0.90),
    ("mon_bat",         "蝙蝠",     3,  26,  0,  2,  4,  13, 10, 12, 1, 7, True,  12, 0,  1,   5, 0.90),

    # ---- 草原 · 中级带（掉 b2 段装备）----
    ("mon_wolf",        "野狼",     4,  60,  1,  3,  6,  22, 4, 12, 1,  7, True,  14, 7,  2,   8, 0.90),
    ("mon_skeleton",    "骷髅",     4,  70,  3,  3,  7,  24, 4, 13, 1,  7, True,  14, 6,  3,   9, 0.90),
    ("mon_spider",      "巨蜘蛛",   5,  66,  2,  4,  8,  30, 8, 10, 1,  7, True,  13, 0,  3,  10, 0.95),
    ("mon_zombie",      "僵尸",     5, 110,  4,  4,  8,  34, 2, 16, 1,  5, True,  12, 5,  4,  12, 1.00),
    ("mon_boar",        "野猪",     6, 110,  4,  5,  9,  40, 6, 14, 1,  6, True,  10, 5,  5,  15, 1.00),
    ("mon_orc",         "半兽人",   6, 130,  5,  6, 10,  46, 5, 13, 1,  7, True,  16, 8,  6,  16, 1.00),

    # ---- 洞窟 · 深处带（掉 b3 段装备）----
    ("mon_ghoul",       "食尸鬼",   7, 150,  5,  7, 12,  60, 5, 12, 1,  7, True,  15, 6,  8,  20, 1.00),
    ("mon_gargoyle",    "石像鬼",   8, 190,  8,  8, 13,  75, 4, 14, 3,  8, True,  16, 0, 10,  24, 1.00),
    ("mon_wolf_alpha",  "狼王",     8, 200,  6, 10, 15,  90, 7, 11, 1,  8, True,  18, 9, 12,  26, 1.00),
    ("mon_wraith",      "幽魂",     9, 180,  4, 13, 18, 110, 6, 11, 4,  9, True,  20, 0, 14,  30, 1.00),
    ("mon_golem",       "石头人",  10, 300, 12, 11, 17, 140, 3, 16, 1,  6, True,  14, 0, 18,  36, 1.00),
    ("mon_dark_knight", "暗黑骑士", 11, 260, 10, 14, 20, 165, 5, 12, 1,  8, True,  18, 7, 22,  44, 1.00),
    ("mon_lich",        "巫妖",    12, 230,  7, 16, 23, 200, 4, 13, 5, 10, True,  20, 0, 28,  52, 1.00),
    ("mon_dragon_whelp","幼龙",    13, 340, 13, 17, 25, 260, 5, 12, 3,  9, True,  20, 0, 36,  64, 1.00),
    ("mon_cave_lord",   "洞窟领主", 15, 560, 16, 21, 31, 460, 4, 11, 2, 11, True,  24, 0, 60, 120, 1.00),
]
...
COLUMNS = ["id", "name", "level", "hp", "ac", "minDc", "maxDc", "exp", "moveSpeed",
...
def build_monsters(items):
    ...
    tier_of = {item_id: tier for item_id, _, _, _, tier in gen_items.TABLE}
    ...
    for row in MONSTERS:
        ...
        drops = []
        for d in pool:
            chance = target * DROP_WEIGHT[tier_of[d["id"]]] / weight_sum
        ...
        out.append({
            ...
            "level": dto["level"], "hp": dto["hp"], "ac": dto["ac"],
...
def validate(monsters, items):
    ...
    # 强度曲线：等级高的怪不该比等级低的弱（比"血量 × 攻击"这个粗口径）
    problems = []
    by_level = sorted(monsters, key=lambda m: m["level"])
    prev = None
    for m in by_level:
        power = m["hp"] * (m["minDc"] + m["maxDc"]) / 2.0
        if prev is not None and m["level"] > prev[0] and power <= prev[1]:
            problems.append("%s(Lv%d) 不比 %s(Lv%d) 强" % (m["name"], m["level"], prev[2], prev[0]))
        prev = (m["level"], power, m["name"])
    if problems:
        for p in problems:
            bad += fail(p)
    else:
        print("  [PASS] 等级越高越强（血量 × 攻击单调递增）")
...
def main():
    ...
    for m in monsters:
        ...
        equip = sum(d["chance"] for d in m["drops"]
                    if d["itemId"] in {i["id"] for i in items if i.get("type") == "equip"})
        print("  %-16s Lv%-2d 血%-4d 攻%-5s %s  射程%d %s%s  爆装 %.3f"
              % (m["id"], m["level"], m["hp"], "%d-%d" % (m["minDc"], m["maxDc"]),
                 band, m["attackRange"],
